...
import com.chen.download.R;
import com.chen.download.Session;
...
class DownloadNotification {
	...
	Context mContext;
	LinkedHashMap<String, NotificationItem> mNotifications;
	...
	static class NotificationItem {
		...
		String mDescription;
		String mTitles; // download titles.
		String mPausedText = null;

		/*
		 * Add a second download to this notification item.
		 */
		void setItem(String title, long currentBytes, long totalBytes) {
	...
	DownloadNotification(Context ctx, SystemFacade systemFacade) {
		...
		mNotifications = new LinkedHashMap<String, NotificationItem>();
	}
	...
	private void updateActiveNotification(Collection<DownloadInfo> downloads) {
		...
		for (DownloadInfo download : downloads) {
			...
			if (mNotifications.containsKey(url)) {
				item = mNotifications.get(url);
				item.setItem(title, progress, max);
			} else {
		...
		for (NotificationItem item : mNotifications.values()) {
			...
			if (VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB) {
				Notification.Builder mNotificationBuilder = new Notification.Builder(
						mContext);
				mNotificationBuilder.setSmallIcon(iconResource);
				mNotificationBuilder.setWhen(System.currentTimeMillis());// 设置时间发生时间
				mNotificationBuilder.setContentTitle(item.mTitles);

				if (hasPausedText) {
					mNotificationBuilder.setProgress(100, 0, true);
					mNotificationBuilder.setContentText(item.mPausedText);
				} else {
					int percent = (int) item.mTotalCurrent * 100
							/ (int) item.mTotalTotal;
					mNotificationBuilder.setProgress(100, percent, false);
				}
				mNotificationBuilder.setContentText(getDownloadingText(
				...
				n = mNotificationBuilder.getNotification();
				...
				expandedView.setTextViewText(R.id.title, item.mTitles);

				if (hasPausedText) {
					expandedView
							.setViewVisibility(R.id.progress_bar, View.GONE);
					expandedView.setTextViewText(R.id.paused_text,
							item.mPausedText);
				} else {
				...
				expandedView
						.setTextViewText(
								R.id.progress_text,
								getDownloadingText(item.mTotalTotal,
										item.mTotalCurrent));
				...
				n.contentIntent = PendingIntent.getBroadcast(mContext, 0,
						intent, 0);
			}
	...
	private void updateCompletedNotification(Collection<DownloadInfo> downloads) {
		for (DownloadInfo download : downloads) {
			...
			if (Downloads.isStatusError(download.mStatus)) {
				...
				Session.get(mContext).getDownloadingList()
						.remove(download.mUri);
